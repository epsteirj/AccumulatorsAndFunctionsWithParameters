...
def run_test_even_better_draw_circles():
    ...
    print('--------------------------------------------------')
    even_better_draw_circles(10,.5,100)
    even_better_draw_circles(20,1.5,300)
    even_better_draw_circles(15, 1, 200)

def even_better_draw_circles(n,x,y):
    window = rg.RoseWindow(400, 400)

    center = rg.Point(y, 200)
    for k in range(n):
        circle = rg.Circle(center, n * k)
        circle.attach_to(window)
        window.render(0.05*x)  # Pauses for 0.05*x seconds after rendering.
